...
export function TokenGateContent({ onSaved }: { onSaved: () => void }) {
  ...
  const field = (key: keyof typeof form, placeholder: string) => <input value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} placeholder={placeholder} type={key === "password" ? "password" : "text"} className="mt-3 w-full sw-input" />;
