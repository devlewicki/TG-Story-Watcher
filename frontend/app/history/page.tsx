    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1">
        {(["views", "activity"] as const).map((t) => (
          ...
          >
            {t === "views" ? "Просмотры историй" : "Журнал активности"}
          </button>
        ))}
      </div>
      ...
      {tab === "views" ? (
        <Card>
          <CardHeader
            title={`${shown < viewsTotal ? `Показано ${shown} из ${viewsTotal}` : `${viewsTotal} просмотров`}`}
          />
          {(views ?? []).length === 0 ? (
            <Empty label="Просмотров историй пока нет" />
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-left text-xs text-slate-500 dark:border-slate-800 dark:text-slate-400">
                    ...
                    <th className="px-4 py-2">ID истории</th>
                    <th className="px-4 py-2">Источник</th>
                    <th className="px-4 py-2">Просмотрена</th>
        ...
        <Card>
          <CardHeader
            title={`${shown < actTotal ? `Показано ${shown} из ${actTotal}` : `${actTotal} событий`}`}
          />
          {(act ?? []).length === 0 ? (
