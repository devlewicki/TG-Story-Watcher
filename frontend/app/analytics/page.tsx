...
export default function AnalyticsPage() {
  ...
  if (!data) return <Empty label={t("analytics.noAnalytics")} />;

  const periods = [
    { value: 1, label: t("analytics.today") },
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-xl font-semibold">{t("analytics.accountStats")}</h1></div><div className="flex gap-2"><div className="flex rounded-xl border border-slate-300 p-0.5 dark:border-slate-700">{periods.map((period) => <button key={period.value} onClick={() => setDays(period.value)} className={`rounded-lg px-3 py-1.5 text-sm ${days === period.value ? "bg-emerald-600 text-white" : "text-slate-500"}`}>{period.label}</button>)}</div><Button onClick={sync} disabled={syncing}>{syncing ? t("analytics.syncing") : t("analytics.sync")}</Button></div></div>
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-6"><StatCard label={t("analytics.storiesLabel")} value={data.stories} /><StatCard label={t("analytics.views")} value={data.views} accent /><StatCard label={t("analytics.viewers")} value={data.known_viewers} /><StatCard label={t("analytics.reactions")} value={data.reactions} accentKey="red" /><StatCard label={t("analytics.forwards")} value={data.forwards} accentKey="sky" /><StatCard label={t("analytics.avgER")} value={`${data.average_er.toFixed(2)}%`} accentKey="indigo" /></div>
    <div className="grid gap-5 lg:grid-cols-2"><Card><CardHeader title={t("analytics.recentActions")} subtitle={t("analytics.recentSubtitle")} />{recent.loading ? <div className="p-5"><Spinner /></div> : recent.data?.length ? <div className="divide-y divide-slate-100 dark:divide-slate-800">{recent.data.map((event, index) => <div key={`${event.story_id}-${event.user_id}-${event.occurred_at}-${index}`} className="flex items-center gap-3 px-5 py-3 text-sm"><span className={`flex h-8 w-8 items-center justify-center rounded-full ${event.type === "reaction" ? "bg-rose-100 dark:bg-rose-500/15" : "bg-sky-100 dark:bg-sky-500/15"}`}>{event.type === "reaction" ? "❤️" : "👁"}</span><div className="min-w-0 flex-1"><div className="truncate"><span className="font-medium">{event.username ? `@${event.username}` : [event.first_name, event.last_name].filter(Boolean).join(" ") || `User ${event.user_id}`}</span><span className="ml-2 text-slate-500">{[event.first_name, event.last_name].filter(Boolean).join(" ") || ""}</span><span className="text-slate-500"> {event.type === "reaction" ? t("analytics.reactedWith", { reaction: event.reaction || "" }) : t("analytics.viewedStory")}</span> <Link className="font-medium text-emerald-600 hover:underline" href={`/analytics/stories/${event.story_id}`}>#{event.telegram_story_id}</Link></div><div className="text-xs text-slate-400">{new Date(event.occurred_at).toLocaleString(locale)}</div></div></div>)}</div> : <Empty label={t("analytics.noRecentActions")} />}</Card>
    <Card><CardHeader title={t("analytics.topStories")} subtitle={days === 3650 ? t("analytics.topStoriesAllTime") : t("analytics.topStoriesPeriod", { days })} />{data.top_stories.length === 0 ? <Empty label={t("analytics.noStoriesSynced")} /> : <div className="divide-y divide-slate-100 dark:divide-slate-800">{data.top_stories.map((story, index) => <Link href={`/analytics/stories/${story.story_id}`} key={story.story_id} className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50"><span className="w-6 text-sm font-semibold text-slate-400">{index + 1}</span><span className="flex-1"><span className="font-medium">Story #{story.telegram_story_id}</span><span className="ml-3 text-xs text-slate-400">{story.published_at ? new Date(story.published_at).toLocaleString(locale) : ""}</span></span><span className="text-sm text-slate-500">👁 {story.views ?? "—"}</span><span className="text-sm text-slate-500">❤️ {story.reactions ?? "—"}</span><span className="w-20 text-right text-sm font-medium text-emerald-600">{story.er == null ? "—" : `${story.er.toFixed(2)}%`}</span></Link>)}</div>}</Card></div>
  </div>;
}
