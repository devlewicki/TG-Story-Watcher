        ...
        <div className="space-y-4 p-5">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <input
                type="range" min={1} max={60} value={intervalMin}
                onChange={(e) => update({ search_interval: Math.max(1, Number(e.target.value)) * 60 })}
                className="h-2 w-full cursor-pointer appearance-none rounded-full bg-slate-200 accent-emerald-600 dark:bg-slate-700"
              />
              <div className="flex justify-between text-[11px] text-slate-400"><span>1</span><span>60</span></div>
              <p className="text-xs text-slate-400">{t("discovery.minIntervalNote")}</p>
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-200">
              ...
              <input
                type="range" min={50} max={200} step={10} value={cfg.search_results_max}
                onChange={(e) => update({ search_results_max: Math.max(50, Number(e.target.value)) })}
                className="h-2 w-full cursor-pointer appearance-none rounded-full bg-slate-200 accent-emerald-600 dark:bg-slate-700"
              />
              <div className="flex justify-between text-[11px] text-slate-400"><span>50</span><span>200</span></div>
            </div>
          </div>
        </div>
      </Card>
