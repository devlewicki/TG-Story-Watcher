...
def dashboard(db:Db,user_id:Annotated[int,Depends(current_user_id)]):
 now=datetime.now(timezone.utc); today=start_day(now); ids=[x.id for x in db.query(TelegramAccount).filter_by(user_id=user_id).all()]
 def c(model,*criteria):
  q=db.query(func.count(model.id)).filter(*criteria); return q.scalar() or 0
 recent=now-timedelta(days=1)
 return {"accounts":{"total":len(ids),"active":len(ids),"monitoring_on":db.query(TelegramAccount).filter(TelegramAccount.id.in_(ids),TelegramAccount.monitoring.is_(True)).count()},"cards":{"accounts":len(ids),"monitoring":db.query(TelegramAccount).filter(TelegramAccount.id.in_(ids),TelegramAccount.monitoring.is_(True)).count(),"viewed_today":c(StoryView,StoryView.account_id.in_(ids),StoryView.viewed_at>=today),"in_queue":c(StoryQueue,StoryQueue.account_id.in_(ids),StoryQueue.status.in_(["PENDING","WAITING_DELAY","PROCESSING"])),"stories":c(Story,Story.account_id.in_(ids)),"skipped":c(ActivityLog,ActivityLog.account_id.in_(ids),ActivityLog.event_type=="story_skipped",ActivityLog.created_at>=recent),"errors":c(ActivityLog,ActivityLog.account_id.in_(ids),ActivityLog.level.in_(["ERROR","CRITICAL"]),ActivityLog.created_at>=recent)},"charts":{"views_by_hour":[{"hour":h,"count":c(StoryView,StoryView.account_id.in_(ids),StoryView.viewed_at>=today+timedelta(hours=h),StoryView.viewed_at<today+timedelta(hours=h+1))} for h in range(24)],"views_by_day":[{"day":(today-timedelta(days=d)).isoformat(),"count":c(StoryView,StoryView.account_id.in_(ids),StoryView.viewed_at>=today-timedelta(days=d),StoryView.viewed_at<today-timedelta(days=d-1))} for d in range(13,-1,-1)]},"recent":[{"id":x.id,"account_id":x.account_id,"event_type":x.event_type,"level":x.level,"message":x.message,"created_at":x.created_at} for x in db.query(ActivityLog).filter(ActivityLog.account_id.in_(ids)).order_by(ActivityLog.created_at.desc()).limit(15)]}
@router.get("/stats")
def stats(db:Db,user_id:Annotated[int,Depends(current_user_id)],days:int=7):
 now=datetime.now(timezone.utc); start=start_day(now)-timedelta(days=max(days-1,0)); ids=[x.id for x in db.query(TelegramAccount.id).filter_by(user_id=user_id)]
 ...
 daily=[{"day":(start_day(now)-timedelta(days=d)).date().isoformat(),"count":count(StoryView,StoryView.account_id.in_(ids),StoryView.viewed_at>=start_day(now)-timedelta(days=d),StoryView.viewed_at<start_day(now)-timedelta(days=d-1))} for d in range(max(days-1,0),-1,-1)]
 hourly=[{"hour":h,"count":count(StoryView,StoryView.account_id.in_(ids),StoryView.viewed_at>=start,StoryView.viewed_at<now)} for h in range(24)]
